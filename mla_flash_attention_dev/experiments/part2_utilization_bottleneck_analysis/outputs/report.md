...
- `24q/28q/32q/40q/48q` 的 corrected-runtime `perf-fpu` boundary points：来自 `deepseek_flash_mla_part2_probe_pm_fix_verify_boundary_perf_fpu_rerun` 与 `deepseek_flash_mla_part2_probe_pm_fix_verify_boundary_mid_perf_fpu_rerun` 的 `TT / DeepSeek-4c / DeepSeek-8c`；`24q` 是已 fit anchor，其余点用于观察跨墙后 `8c` 的直接算术利用率变化。
