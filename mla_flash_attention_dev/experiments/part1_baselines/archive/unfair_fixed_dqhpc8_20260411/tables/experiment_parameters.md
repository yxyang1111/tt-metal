...
| probe preset | `wide_bh_full` | capability probe 使用的 preset；`manual` 表示直接读取 CLI 轴参数 |
| device_id | `0` | TT 设备编号 |
...
| capability probe | `true` | status=`completed`；汇总见 `tables/capability_probe_summary.md`，原始结果见 `raw/capability_probe_results.json` |
...
| `--run-capability-probe` | `true` | 当前工作区是否存在 capability probe 结果 |
