...
| default config | `b6_h32_hkv1_dv512_ro64_blk64_kc128_dqhpc8` | 默认主表固定使用的 config |
| default config label | `B=6, H=32, H_kv=1, value_dim=512, rope_dim=64, block=64, k_chunk=128, deepseek_q_heads_per_core=8` | 默认主表的人类可读说明 |
...
| warmup_device / iters_device | `10 / 20` | TT baseline 预热 / 正式测量次数 |
...
| `deepseek_num_q_heads_per_core` | `8` | DeepSeek Q shard 粒度 sweep / 派生结果 |
...
| 项目 | 当前值 | 说明 |
|---|---|---|
| decode q_num_cores | `56` | TT 主线 decode 默认 `q_num_cores=min(B*H, grid_x*grid_y)` |
| DeepSeek num_q_shards | `4` | `num_heads / deepseek_num_q_heads_per_core` |
| DeepSeek required q cores | `24` | 当前 builder 使用 `batch * num_q_shards` 个活跃核 |
