...
- `mla_flash_attention_dev/experiments/run_flash_mla_pm_bubble_probe.py`
...
```bash
python3 mla_flash_attention_dev/experiments/run_flash_mla_pm_bubble_probe.py --smoke
```
...
```bash
python3 mla_flash_attention_dev/experiments/run_flash_mla_pm_bubble_probe.py --seq-lens 256 512 1k 2k 4k 8k 16k 32k
```

先看命令不真正执行：

```bash
python3 mla_flash_attention_dev/experiments/run_flash_mla_pm_bubble_probe.py --dry-run
```
...
```bash
python3 mla_flash_attention_dev/experiments/run_flash_mla_pm_bubble_probe.py --force
```
...
python3 -m tracy -v -r -p \
  -o "mla_flash_attention_dev/experiments/profile_outputs/flash_mla_pm_bubble_probe/decode_1k" \
  --check-exit-code \
  ...
  -a device_compute_cb_wait_front \
  -a device_compute_cb_reserve_back \
  mla_flash_attention_dev/experiments/profile_flash_mla_wh_detailed.py --child-run --case decode_1k
```
