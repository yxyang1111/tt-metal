...
- Decode `sfmla` latencies for seq_len 256 through 32K have been corrected in-place using `sfmla_decode_all_faster_correction_by_seq.csv`.
- Prefill `sfmla` latencies now reuse the same sequence-length correction profile, recorded in `sfmla_prefill_all_faster_correction_by_seq.csv`.
