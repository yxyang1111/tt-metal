...
- long-seq 代理边界与 Part II / III 一致：`decode_4k / 16k / 32k` 的 `DeepSeek / TT` ratio 为 `1.025x / 0.976x / 0.997x`，最大偏差约 `2.5%`。
- 但 `decode_1k` 上 `DeepSeek / TT = 0.864x`，因此 short-seq 架构投影不能直接解释成 DeepSeek 的 direct future-hardware prediction。

## 2. 数据来源

- A-BH 二阶经验模型：`part1_three_baselines/outputs/flashmla_detailed/flash_mla_wh_detailed_profile_results.json` 中的 `bh_empirical_decode`。
...
- native `B-BH` 的主瓶颈在当前五个代表点上都已经转成 `k_mcast`，并且相对 A-BH empirical 的优势会从 `decode_1k` 的 `1.28x` 持续扩大到 `decode_32k` 的 `5.60x`。
...
- 汇总 JSON：`raw/part4_architecture_implications.json`
