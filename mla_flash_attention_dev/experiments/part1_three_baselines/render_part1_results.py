...
def backfill_config_metadata(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    metadata = dict(payload.get("metadata", {}))
    if results and all("config_signature" in row for row in results if row.get("status") == "ok") and metadata.get("default_config"):
        return payload
    ...
    metadata.setdefault("configs", [default_config.to_dict()])
    metadata.setdefault("default_config", default_config.to_dict())
    metadata.setdefault(
        "decode_seq_lens",
...
def make_experiment_parameter_table(payload: dict[str, Any], detail_payload: dict[str, Any] | None) -> str:
    metadata = payload.get("metadata", {})
    results = payload.get("results", [])
    ...
    lines = [
        ...
        f"| probe preset | `{metadata.get('probe_preset', 'manual')}` | capability probe 使用的 preset；`manual` 表示直接读取 CLI 轴参数 |",
        ...
        f"| capability probe | `{str(bool(metadata.get('run_capability_probe'))).lower()}` | probe 轴与结果详见 `raw/capability_probe_results.json` |",
        "",
        "## 3. 默认 config 显式字段",
        "",
        "| 字段 | 当前值 | 说明 |",
        ...
        f"| `--run-capability-probe` | `{str(bool(metadata.get('run_capability_probe'))).lower()}` | 是否先跑公共参数空间 probe |",
...
def same_except_axis(config: ExperimentConfig, default: ExperimentConfig, axis: str) -> bool:
    if axis == "batch":
        return (
...
def axis_label(config: ExperimentConfig, axis: str) -> str:
    if axis == "batch":
...
def make_axis_sweep_table(
    ...
    *,
    ...
    candidates = sorted(
        [config for config in configs if same_except_axis(config, default_config, axis)],
    ...
    labels = [axis_label(config, axis) for config in candidates]
    lines = [
        f"# {title}",
        "",
...
def build_report(
    ...
    lines.append(f"- Capability probe 表：`{CAPABILITY_PROBE_MD.relative_to(OUTPUT_DIR)}`")
...
<body>
  <div class="wrap">
    ...
    <p>当前目录聚合了四方法 decode benchmark、三方法 prefill control、延迟/吞吐量图表，以及可选的 `FlashMLA（TT 主线）` detailed profile 结果。</p>
    <div class="grid">
      {''.join(image_blocks)}
    </div>
  </div>
...
def main() -> None:
    ...
    results_map = results_by_key(payload["results"])
    write_text(PARAMETER_TABLE_MD, make_experiment_parameter_table(payload, detail_payload))
    ...
    dashboard_paths = {**visual_paths, **{key: value for key, value in multidim_artifacts.items() if key.endswith("_latency")}}
    write_text(REPORT_MD, build_report(payload, detail_payload, probe_payload, visual_paths, multidim_artifacts))
